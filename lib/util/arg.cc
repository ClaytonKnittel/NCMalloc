...
static const char * usage_fmt = "%16.20s,%8.8s,%16.16s,%10.10s,\t%s";
...
argument_parser::parse_arguments(uint32_t argc, char ** cmdline) {
    ...
    argument_info * cur_argument = NULL;

    DIE_ASSERT(argc >= 1, "Impossible condition (this is mostly to satisfy the linter\n");
    for (uint32_t i = 1; i < argc; i++) {
        if (expecting_value) {
            ...
            else if(cur_argument->get_var() == NULL && cur_argument->get_type() != Help) {
                this->exit_usage_error(ERROR_PARSING_CMDLINE,
                                       "Error argument \"%s\" assosiated with invalid variable\n",
                                       cmdline[i]);
            }
