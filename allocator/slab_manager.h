...
struct slab_manager {
    ...
    uint32_t
    _append_free_slab(const uint32_t start_cpu, slab_t * slab) {
        static_assert(slab_t::next_offset == OBJ_SLAB_NEXT_OFFSET);
        SM_DBG_ASSERT(slab != NULL);
        SM_DBG_ASSERT(slab->next == NULL);
...
#pragma GCC diagnostic push
        ...
        return 0;
    abort:
        /* https://gcc.gnu.org/onlinedocs/gcc-6.5.0/gcc/Label-Attributes.html
         * says this is okay... */
        COLD_ATTR;  // NOLINT
        return ANY;
    }
